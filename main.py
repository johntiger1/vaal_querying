...
def rl_main(args):
    ...
    STATE_SPACE = 1
    ACTION_SPACE = args.num_images

    task_model = model.FCNet(num_classes=args.num_classes)

    pol_net = PolicyNet(STATE_SPACE , ACTION_SPACE ) # gradient, or hessian in the network..; per class accs as well
    pol_optimizer = optim.Adam(pol_net.parameters(), lr=5e-3)
    args.num_episodes = 10000
    curr_state = torch.zeros((STATE_SPACE,1)) #only feed it in the past state directly
    dynamic_action_mask = torch.ones((ACTION_SPACE, 1))

    for i in range(args.num_episodes):
        pol_optimizer.zero_grad()
        action_vector = pol_net(curr_state )
        action_dist = torch.distributions.Categorical(action_vector) * dynamic_action_mask
        ...
        dynamic_action_mask[action ] = 0 #update the dynamic action mask
